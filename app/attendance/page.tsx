...
import { getEvents, type EventRow } from "@/app/actions/events";
import {
...
export default function AttendancePage() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      if (!canManageAttendance(currentSession)) {
        setIsLoading(false);
        return;
      }

      const [sessionRes, statRes, eventRes] = await Promise.all([
        getAttendanceSessions(),
        getAttendanceStats(),
        getEvents(),
      ]);
      ...
      const nextSessions =
        sessionRes.success && sessionRes.sessions ? sessionRes.sessions : [];
      ...
      if (eventRes.success && eventRes.events) {
        setEvents(
          (eventRes.events as EventRow[]).map((event) => ({
            id: event.id,
            title: event.title,
            date: event.date,
            time: event.time,
      ...
      if (nextSessions[0]) {
        await loadDetails(nextSessions[0].id);
      }
    }
    ...
    <div className="min-h-screen bg-purple-50/30">
      <header className="border-b border-purple-200/70 bg-gradient-to-r from-violet-50/80 via-purple-50/70 to-fuchsia-50/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          <button
            onClick={openNew}
            className="w-fit rounded-lg bg-purple-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-purple-700"
          >
            + New Attendance
          </button>
        </div>
